...
public class CustomParser {
	...
	public static boolean checkMethodAnnotation(String methodName, Class<?>[] paramTypes, String expectedValue) {
		try {
			Method method = com.yaksha.assignment.controller.UserInfoController.class.getMethod(methodName, paramTypes);
	...
	// Method to check if the method correctly handles request headers (like
	// @RequestHeader)
	public static boolean checkControllerMethodForRequestHeader(String simulatedUserAgent, String headerName)
			throws IOException {
		// Similar logic as checkControllerMethodForMapping() for request headers
		String controllerClassPath = "src/main/java/com/yaksha/assignment/controller/UserInfoController.java";
		String classContent = loadClassContent(controllerClassPath);
		if (classContent == null) {
			return false;
		}
		...
		// Get the controller class declaration
		Optional<ClassOrInterfaceDeclaration> classDeclaration = compilationUnit.getClassByName("UserInfoController");
		if (classDeclaration.isEmpty()) {
			return false;
		}

		// Iterate through the methods to find the one with the correct @RequestHeader
		for (MethodDeclaration method : classDeclaration.get().getMethods()) {
			if (method.getAnnotations().stream().anyMatch(a -> a.getNameAsString().equals("RequestHeader"))) {
				// Check if the method uses the correct header name
				for (String param : simulatedUserAgent.split(";")) {
					if (method.getParameters().stream().anyMatch(p -> p.getType().asString().contains("String")
							&& p.getName().asString().equals(headerName))) {
						return true;
					}
				}
			}
	...
	// Method to check if a controller method is correctly mapped with
	// @RequestMapping or @GetMapping
	public static boolean checkControllerMethodForMapping(String simulatedRequestUrl, String... paramNames)
			throws IOException {
		String controllerClassPath = "src/main/java/com/yaksha/assignment/controller/UserInfoController.java"; // Adjust
																												// this
																												// path
																												// to
																												// your
		...
		// Iterate through the methods to find the one with the correct @GetMapping or
		// @RequestMapping
		for (MethodDeclaration method : classDeclaration.get().getMethods()) {
			boolean hasCorrectMapping = false;
			if (method.getAnnotations().stream().anyMatch(
					a -> a.getNameAsString().equals("GetMapping") || a.getNameAsString().equals("RequestMapping"))) {
				// Checking if the URL mapping contains the simulated request URL
				if (method.getAnnotationByName("GetMapping").isPresent()) {
					String mappingValue = method.getAnnotationByName("GetMapping").get().toString();
					if (mappingValue.contains(simulatedRequestUrl)) {
						hasCorrectMapping = true;
					}
				}

				// Check if method has parameter names matching the expected query parameters
				for (String param : paramNames) {
					if (method.getParameters().stream().anyMatch(p -> p.getName().asString().equals(param))) {
						hasCorrectMapping = hasCorrectMapping && true;
					} else {
			...
			if (hasCorrectMapping) {
				return true; // Method is correctly mapped
			}
		}
		return false; // No matching mapping found
	}

	// Method to check if the controller method handles @RequestParam for the given
	// parameters
	public static boolean checkControllerMethodForRequestParam(String simulatedRequestUrl, String... paramNames)
			throws IOException {
		String controllerClassPath = "src/main/java/com/yaksha/assignment/controller/UserInfoController.java"; // Adjust
																												// this
																												// path
		...
		// Iterate through the methods to find the one with the correct @RequestParam
		for (MethodDeclaration method : classDeclaration.get().getMethods()) {
			boolean hasCorrectMapping = false;
			if (method.getAnnotations().stream().anyMatch(a -> a.getNameAsString().equals("RequestParam"))) {
				// Check if the method has parameters matching the expected query parameters
				for (String param : paramNames) {
					if (method.getParameters().stream().anyMatch(p -> p.getName().asString().equals(param))) {
						hasCorrectMapping = true;
					} else {
			...
			if (hasCorrectMapping) {
				return true; // Method is correctly handling @RequestParam for the expected parameters
			}
		}
		return false; // No method found handling the given request parameters
	}

	// Checks if the response (JSP file) contains the expected parameter value in
	// the rendered HTML
	public static boolean checkResponseForParameterValue(String filePath, String expectedValue) throws IOException {
		// Read the content of the JSP file
		File file = new File(filePath);
		if (!file.exists()) {
			System.out.println("Error: JSP file not found at path - " + filePath);
			return false;
		}

		// Read the file as a String (HTML content)
		String jspContent = new String(Files.readAllBytes(Paths.get(filePath)));

		// Check if the expected value is present in the content
		boolean containsExpectedValue = jspContent.contains(expectedValue);

		if (containsExpectedValue) {
			System.out.println("Success: Found the expected value: " + expectedValue + " in " + filePath);
		} else {
			System.err.println("Error: Expected value " + expectedValue + " not found in " + filePath);
		}
